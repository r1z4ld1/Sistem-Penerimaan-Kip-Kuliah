...
@section('content')
    ...
    <div class="mb-6 lg:mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
            ...
            <p class="text-slate-500 mt-1.5 text-sm lg:text-base">
                Kelola daftar universitas atau institusi pendidikan yang terdaftar di dalam sistem.
            </p>
        </div>

        {{-- Action Button --}}
        @can('create universitas')
            <div class="shrink-0">
                <a href="{{ route('admin.universitas.create') }}"
    ...
    <div class="mb-6 flex justify-between items-center">
        <form method="GET" class="w-full md:max-w-md relative">
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama universitas..."
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                <thead class="bg-slate-50 border-b border-slate-200/80">
                    <tr>
                        <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider w-1/3">Nama
                            Universitas</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($universitas as $item)
                        <tr class="hover:bg-slate-50/50 transition-colors align-middle">
                            ...
                            {{-- Nama Universitas --}}
                            <td class="px-6 py-4">
                                <span class="text-sm font-bold text-slate-800 block">{{ $item->nama_universitas }}</span>
                            </td>

                            {{-- Alamat --}}
                            <td class="px-6 py-4">
                                <span class="text-sm font-medium text-slate-600">
                                    {{ $item->alamat ?: 'Belum ada data alamat' }}
                                </span>
                            </td>
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    @can('delete universitas')
                                        <form action="{{ route('admin.universitas.destroy', $item->id) }}" method="POST"
                                            class="inline-block"
                                            onsubmit="return confirm('Apakah Anda yakin ingin menghapus data universitas ini?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                        ...
                        <tr>
                            <td colspan="3" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <h3 class="text-sm font-bold text-slate-800 mb-1">Tidak Ada Data Universitas</h3>
                                    <p class="text-xs text-slate-500 max-w-xs mx-auto">
                                        {{ request('search') ? 'Tidak ada hasil pencarian yang cocok.' : 'Belum ada data universitas yang ditambahkan ke sistem.' }}
                                    </p>
                                </div>
                            </td>
                        </tr>
