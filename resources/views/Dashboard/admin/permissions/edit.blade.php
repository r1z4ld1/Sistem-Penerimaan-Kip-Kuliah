...
@section('content')
    ...
    <div class="mb-6 lg:mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-slate-500 mt-1.5 text-sm lg:text-base">
                Perbarui informasi nama permission yang ada di dalam sistem.
            </p>
        </div>
    ...
    <div class="max-w-2xl bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        <form action="{{ route('admin.permissions.update', $permission->id) }}" method="POST" class="p-6 md:p-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <p class="text-[11px] text-slate-500 mt-2">
                    Gunakan format teks yang konsisten (misalnya <code>gudang.create</code> atau <code>manage users</code>)
                    agar sesuai dengan aturan sistem <i>helper</i> Anda.
                </p>
            </div>
