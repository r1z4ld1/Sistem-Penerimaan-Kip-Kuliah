...
@section('content')
    ...
    <div class="mb-6 lg:mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-slate-500 mt-1.5 text-sm lg:text-base">
                Tambahkan program studi baru dan hubungkan dengan universitas induknya.
            </p>
        </div>
    ...
    <div class="max-w-4xl bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        <form action="{{ route('admin.jurusan.store') }}" method="POST" class="p-6 md:p-8">
            @csrf
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        <select name="universitas_id"
                            ...
                            <option value="" disabled {{ old('universitas_id') ? '' : 'selected' }}>
                                Pilih Universitas
                            </option>
                            @foreach ($universitas as $item)
                                <option value="{{ $item->id }}"
                                    {{ old('universitas_id') == $item->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="text" name="nama_jurusan" value="{{ old('nama_jurusan') }}"
                        placeholder="Masukkan nama resmi program studi (contoh: Teknik Informatika)"
